...
from core.patches import DomainModulationPatch


class ParametrizedModulatedConv2d(nn.Module):
    def __init__(
            self,
            conv_to_patch
    ):
        ...
        self.domain_modulation = DomainModulationPatch(
            conv_to_patch.weight
        )

    def forward(self, input, style, params=None):
...
class ParametrizedStyledConv(nn.Module):
    def __init__(
            self,
            in_channel,
            out_channel,
...
class ParametrizedGenerator(nn.Module):
    def __init__(
            self,
            size,
            style_dim,
        ...
        self.conv1 = ParametrizedStyledConv(
        ...
        for layer_idx in range(self.num_layers):
            ...
            self.noises.register_buffer('noise_{}'.format(layer_idx),
                                        torch.randn(*shape))
    ...
    def forward(
        ...
        if noise is None:
            if randomize_noise:
                ...
                noise = [getattr(self.noises, 'noise_{}'.format(i)) for i in
        ...
        out = self.conv1(out, latent[:, 0],
                         params=params[
                             'conv_0'] if params is not None else None,
                         noise=noise[0])
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            out = conv1(out, latent[:, i],
                        params=params[
                            f'conv_{i}'] if params is not None else None,
                        noise=noise1)
            out = conv2(out, latent[:, i + 1],
                        params=params[
                            f'conv_{i + 1}'] if params is not None else None,
                        noise=noise2)
