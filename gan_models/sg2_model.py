...
from gan_models.op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d
...
class Upsample(nn.Module):
    ...
    def forward(self, input):
        out = upfirdn2d(input, self.kernel, up=self.factor, down=1,
                        pad=self.pad)
...
class Downsample(nn.Module):
    ...
    def forward(self, input):
        out = upfirdn2d(input, self.kernel, up=1, down=self.factor,
                        pad=self.pad)
...
class EqualConv2d(nn.Module):
    def __init__(
            self, in_channel, out_channel, kernel_size, stride=1, padding=0,
            bias=True
    ):
        super().__init__()
    ...
    def __repr__(self):
        return (
                '{}({}, {},'.format(self.__class__.__name__,
                                    self.weight.shape[1],
                                    self.weight.shape[0]) +
                ' {}, stride={}, padding={})'.format(self.weight.shape[2],
...
class EqualLinear(nn.Module):
    ...
    def __repr__(self):
        return (
            '{}({}, {})'.format(self.__class__.__name__, self.weight.shape[1],
                                self.weight.shape[0])
        )
    ...
    def __repr__(self):
        return (
                '{}({}, {}, {}, '.format(self.__class__.__name__,
                                         ...
                                         self.kernel_size) +
                'upsample={}, downsample={})'.format(self.upsample,
                                                     self.downsample)
        )
...
class Discriminator(nn.Module):
    ...
    def forward(self, input):
        ...
        stddev = out.view(
            group, -1, self.stddev_feat, channel // self.stddev_feat, height,
            width
        )
